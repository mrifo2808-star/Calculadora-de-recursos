  ...
  return (
    <>
      <p className="panel__hint">
        ...
        recursos. Se reutilizan entre proyectos, por eso cada fila se desactiva en vez de eliminarse. Un cargo{' '}
        <strong>Fijo</strong> usa Cantidad × Frecuencia × HH unitarias, igual que un recurso de Cubicación; uno{' '}
        <strong>% Proyecto</strong> es un porcentaje fijo del total de HH de producción del proyecto (recursos de
        Cubicación, etapas activas, por curso — hoy {fmt(baseGestionHH)} HH) y se recalcula solo cuando ese total
        cambia.
      </p>
      <div className="tabla-scroll">
        <table className="tabla">
          ...
          <tbody>
            {calculadas.map((r) => {
              const esPorcentaje = r.tipo === 'porcentaje';
              return (
                <tr key={r.rowId} className={r.activa === false ? 'fila--desactivada' : undefined}>
                  ...
                  <td className={esPorcentaje ? 'num' : undefined}>
                    {esPorcentaje ? (
                      '—'
                    ) : (
                      <input
                  ...
                  <td className={esPorcentaje ? undefined : 'num'}>
                    {esPorcentaje ? (
                      <input
                        type="number"
                        min={0}
